...
# much of this code was modified from https://automaticaddison.com/linear-quadratic-regulator-lqr-with-python-code-example/
# as the robot problem solved in that example is extremely similar to the Zamboni problem here
max_linear_velocity = 3.0 # meters per second
max_angular_velocity = 1.5708 # radians per second
def b_matrix(yaw, deltat):
    """
...
def main():
    ...
    # [meters, meters, radians]
    desired_state_xf = np.array([[1,51.5,np.pi/2], [8.5, 59, 0], [16, 51.5, -np.pi/2]])  

    # Actual state
    # Our robot starts out at the origin (x=0 meters, y=0 meters), and 
    # the yaw angle is 0 radians. 
    start_actual_state_x = np.array([[1,8.5,np.pi/2], [1,51.5,np.pi/2], [8.5, 59, 0]]) 
    ...
    for j in range(len(start_actual_state_x)):
        for i in range(1000):
            if i == 0:
                actual_state_x = start_actual_state_x[j]
            print(f'iteration = {i} seconds')
            print(f'Current State = {actual_state_x}')
            print(f'Desired State = {desired_state_xf[j]}')
            
            state_error = actual_state_x - desired_state_xf[j]
            state_error_magnitude = np.linalg.norm(state_error)     
            print(f'State Error Magnitude = {state_error_magnitude}')
            
            B = b_matrix(actual_state_x[2], dt)
